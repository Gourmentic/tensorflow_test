...
bicluster_data = datasets.make_biclusters(shape=(100,2),n_clusters=2,noise=20)
...
# training part
def train_model(x_train,y_train,gamma_value=0.05,epochs=1000):
    ...
    support_x2 = np.broadcast_to(
        support_x2,
        shape=(
            support_size,
            support_size,
            feature_dim))
    support_kernel = np.exp(
        (-gamma_value) * np.sum((support_x1 - support_x2)**2, axis=2))
    ...
    b_cal = np.mean(1 / y_support - (support_weight *
                                     y_support * support_kernel_sum))


    return x_support,y_support,support_weight,b_cal


x_support,y_support,support_weight,b_cal = train_model(x_train,y_train)


train_1 = (y_train == 1).reshape(-1)
train_0 = (y_train == -1).reshape(-1)
support_1 = (y_support == 1).reshape(-1)
support_0 = (y_support == -1).reshape(-1)
# plt.plot(x_train[train_1, 0], x_train[train_1, 1], '*r')
# plt.plot(x_train[train_0, 0], x_train[train_0, 1], '*b')
# plt.plot(x_support[support_1,0],x_support[support_1,1],'or')
# plt.plot(x_support[support_0,0],x_support[support_0,1],'ob')
# plt.show()
...
def predict_model(x_support,y_support,weight_support,b_cal,x_test,y_test=None,gamma=0.05):
...
y_train_prd = predict_model(x_support,y_support,support_weight,b_cal,x_train,y_train)
...
xx1, xx2 = np.meshgrid(
    np.linspace(
        xx1_min, xx1_max, 1000), np.linspace(
            xx2_min, xx2_max, 1000))
...
plt.plot(x_support[support_1,0],x_support[support_1,1],'or',label='support +1')
plt.plot(x_support[support_0,0],x_support[support_0,1],'ob',label='support -1')
